...
public import MathlibStaging.Algebra.Category.ModuleCat.Topology.AnnihilatorStalk
public import MathlibStaging.Algebra.Category.ModuleCat.Topology.SheafifyInstances
public import Mathlib.Algebra.Category.ModuleCat.Sheaf.Generators
public import MathlibStaging.Algebra.Category.ModuleCat.Sheaf.LocallySurjective
...
set_option backward.isDefEq.respectTransparency false in
/-- The local-generation property for an *explicit* generating datum: an index type `ι`, a chart
`U₀`, a family of target sections `t`, and a generating epimorphism `π` over the slice site whose
basis sections restrict to the `t i`. Splitting this off keeps its proof abstract in the datum, so
elaboration does not repeatedly unfold the (expensive) `σ.generators i₀` construction. -/
private lemma localGeneration_of_sections {ι : Type u} [Fintype ι] {U₀ : Opens X}
    (t : ι → M.val.obj (op U₀)) (π : free (R := R.over U₀) ι ⟶ M.over U₀) [Epi π]
    (hπt : ∀ i, (sectionsMap π (freeSection (R := R.over U₀) i)).eval (op (Over.mk (𝟙 U₀))) = t i)
    ⦃V : Opens X⦄ (hVU₀ : V ≤ U₀) (m : M.val.obj (op V)) ⦃y : X⦄ (hyV : y ∈ V) :
    ∃ (W : Opens X) (hWV : W ≤ V) (_ : y ∈ W) (a : ι → R.obj.obj (op W)),
      M.val.presheaf.map (homOfLE hWV).op m =
        ∑ i, a i • M.val.presheaf.map (homOfLE (le_trans hWV hVU₀)).op (t i) := by
  obtain ⟨W, hWV, hyW, a, ha⟩ := M.exists_localGeneration_of_epi π hVU₀ m hyV
...
theorem exists_localGeneration_of_isFiniteType [M.IsFiniteType] (x : X) :
    ∃ (ι : Type u) (_ : Fintype ι) (U₀ : Opens X) (_ : x ∈ U₀) (t : ι → M.val.obj (op U₀)),
      ∀ ⦃V : Opens X⦄ (hVU₀ : V ≤ U₀) (m : M.val.obj (op V)) ⦃y : X⦄ (_ : y ∈ V),
        ∃ (W : Opens X) (hWV : W ≤ V) (_ : y ∈ W) (a : ι → R.obj.obj (op W)),
          M.val.presheaf.map (homOfLE hWV).op m =
            ∑ i, a i • M.val.presheaf.map (homOfLE (le_trans hWV hVU₀)).op (t i) := by
  -- Extract a finite local generation datum and a chart `U₀ = σ.X i₀ ∋ x`, then hand the abstract
  -- datum to `localGeneration_of_sections`.
  ...
  haveI : (σ.generators i₀).IsFiniteType := hσ.isFiniteType i₀
  haveI : Fintype (σ.generators i₀).I := Fintype.ofFinite _
  exact ⟨(σ.generators i₀).I, inferInstance, σ.X i₀, hxi₀,
    fun i => ((σ.generators i₀).s i).eval (op (Over.mk (𝟙 (σ.X i₀)))),
    M.localGeneration_of_sections _ (σ.generators i₀).π
      fun i => congrArg (·.eval (op (Over.mk (𝟙 (σ.X i₀)))))
        (sectionsMap_freeHomEquiv_symm_freeSection (σ.generators i₀).s i)⟩
